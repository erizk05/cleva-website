...
const regexContainer = /<div style="margin-bottom: 2rem; display: flex; gap: 1rem; flex-wrap: wrap; align-items: center; justify-content: center;">[\s\S]*?<\/table>\s*<\/div>/;
...
html = html.replace(regexContainer, newHTML);

const jsRegex = /function updateTable\(\) \{[\s\S]*?document\.getElementById\('model1'\)\.addEventListener\('change', updateTable\);/

const newJs = `function updateTable() {
        const models = [
...
console.log('Update layout to side-by-side cards');
